...
fn main(){
    ...
    let encoded_data=data.clone().encode_categoricals().return_time_sequenced().collect().unwrap();
    let data_fraction=encoded_data.clone().sample_frac(
        &Series::new("fraction".into(),[1]), false, false, Some(42)).unwrap();
    let batches=encoded_data.height();
    let cols=encoded_data.return_x_columns();
    println!("{:?}", cols);
    let data=encoded_data.select_sequence(cols, batches);
    println!("{:?}", data);
    ...
    // let model_config=lstm::NrelConfig::new(AdamWConfig::new());
