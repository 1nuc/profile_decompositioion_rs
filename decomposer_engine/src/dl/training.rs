use burn::{config::Config, data::dataloader::{DataLoader, DataLoaderBuilder, batcher::Batcher}, module::Module, optim::AdamWConfig, record::CompactRecorder, tensor::backend::AutodiffBackend, train::{Learner, SupervisedTraining,metric::LossMetric}, record::Recorder};
use polars::{frame::DataFrame};
use std::{fmt::Debug, fs::*, path::Path, sync::Arc};

use crate::dl::{dataset::{NrelBatcher, NrelDataset}, models::{bi_lstm::NucBiLstmConfig, hybrid_models::{Seq2SeqConfig, Seq2SeqRecord}, lstm::NucLstmConfig, stacked_bi_lstm::StackedBiLstmConfig, stacked_lstm::StackedLstmConfig}};
...
impl NrelConfig{
    ...
    fn create_artifact_dir<B: AutodiffBackend>(&self,artifact_dir: &str, train_data: DataFrame, test_data: DataFrame, device: B::Device){
    ...
    {
        self.create_artifact_dir::<B>(artifact_dir, train_data.clone(), test_data.clone(), device.clone());
        let model=self.model.init::<B>(device);
        let (train_loader, test_loader)=self.prepare_training(train_data, test_data, &device);
    ...
    pub fn inference_learning<B: AutodiffBackend>(&self,artifact_dir: &str, train_data: DataFrame, test_data: DataFrame,device: B::Device){
        ...
        let record: Seq2SeqRecord<B>= CompactRecorder::new().load(
            format!("{artifact_dir}/model").into(), &device).expect("First batch is not trained yet");
    ...
    fn prepare_training<B: AutodiffBackend> 
        (&self,
         ...
         test_data: DataFrame,
         device: &B::Device) ->
            (Arc<dyn DataLoader<NrelBatcher<B>::Batch>>,
             Arc<dyn DataLoader<<NrelBatcher<B::InnerBackend> as Batcher>::Batch>>){
